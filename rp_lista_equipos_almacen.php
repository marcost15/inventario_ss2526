...
$n=sql2value("SELECT COUNT(*) FROM articulos_entradas
							WHERE estatus_id = 1 AND serial <> '' ORDER BY articulo_id");
$indice=array();
$li=0;
while($li<$n)
{
...
function bd_buscar_equipos_almacen($li)
{
	...
	$sql = "SELECT articulo_id,serial FROM articulos_entradas
							WHERE estatus_id = 1 AND serial <> '' ORDER BY articulo_id" ."$limite";
